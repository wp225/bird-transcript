#!/usr/bin/env python3
"""Render the Canary Transcript player as MP4 videos with animated subtitles.

Reproduces the web demo's animation: a scrolling playhead over the spectrogram,
the currently-sounding syllable highlighted in the token timeline, and a
karaoke-style transcript band that lights up word by word.
...
CARD_Y0, CARD_Y1 = 88, 556
PAD = 22
SPEC_X0 = CARD_X0 + PAD
SPEC_X1 = CARD_X1 - PAD
SPEC_W = SPEC_X1 - SPEC_X0
SPEC_Y0, SPEC_H = 148, 252
SPEC_Y1 = SPEC_Y0 + SPEC_H
TOK_Y0 = SPEC_Y1 + 16
TOK_ROW_H = 28
TOK_ROWS_MAX = 3
PROG_Y = 518
SUB_Y0, SUB_Y1 = 576, 696

f_label = F("DejaVuSans-Bold.ttf", 13)
f_title = F("DejaVuSans-Bold.ttf", 25)
...
f_sub = F("DejaVuSans-Bold.ttf", 30)
...
def draw_static(sample, spec_img, dur, colors, boxes, n_rows):
    ...
    _title = "BENGALESE FINCH TRANSCRIPT" if str(sample.get("bird","")).lower().startswith("bird") else "CANARY TRANSCRIPT"
    d.text((CARD_X0, 26), _title, font=f_label, fill=ACCENT2)
    ...
    # Progress track and its baseline label
...
def render(sample, out_path):
    ...
    # Wrap the transcript into centred subtitle lines, keeping token indices
    sub_lines, line, lw = [], [], 0
    space = text_w(probe, " ", f_sub)
    for i, t in enumerate(tokens):
        w = text_w(probe, t["label"], f_sub)
        if line and lw + space + w > CARD_X1 - CARD_X0 - 48:
            sub_lines.append((line, lw))
            line, lw = [], 0
        lw += (space if line else 0) + w
        line.append((i, t["label"], w))
    if line:
        sub_lines.append((line, lw))
    ...
    for fi in range(n_frames):
        ...
        # Subtitle band, karaoke highlight on the active syllable
        d.rounded_rectangle([CARD_X0, SUB_Y0, CARD_X1, SUB_Y1], 14,
                            fill=SURFACE2 + (255,), outline=BORDER + (255,), width=1)
        total_h = len(sub_lines) * 42
        sy = SUB_Y0 + ((SUB_Y1 - SUB_Y0) - total_h) / 2
        for line, lw in sub_lines:
            x = (CARD_X0 + CARD_X1) / 2 - lw / 2
            for i, label, w in line:
                if i == active:
                    col = colors[label]
                    d.rounded_rectangle([x - 8, sy - 4, x + w + 8, sy + 38], 7, fill=col + (60,))
                    d.text((x, sy), label, font=f_sub,
                           fill=tuple(min(255, int(c * 1.3 + 40)) for c in col) + (255,))
                else:
                    d.text((x, sy), label, font=f_sub, fill=MUTED + (255,))
                x += w + space
            sy += 42
...
def is_canary(sample):
    """Check if the sample corresponds to a canary."""
    bird = str(sample.get("bird", "")).lower()
    bird_name = str(sample.get("bird_name", "")).lower()
    return "canary" in bird or "canary" in bird_name or not bird.startswith("bird")


def has_exact_dahn_cv(sample):
    """Check if any token in the sample has cv_label exactly equal to 'd+ah+n'."""
    for tok in sample.get("tokens", []):
        if tok.get("cv_label") == "d+ah+n":
            return True
    return False
...
def main():
    ...
    # Skip canary clips where any token has cv_label == "d+ah+n"
    samples = [
        s for s in samples
        if not (is_canary(s) and has_exact_dahn_cv(s))
    ]
